...
storiesOf('@jjordy/Layout/Select', module)
  ...
  .add('Error', () => <Select options={options} name='test' label='Hello There' placeholder='Shit theres an error...' error />)
  ...
  .add('Inline', () => (
    <Segment color='primary' rounded={false}>
      ...
      <div style={{ display: 'flex' }}>
        <Select options={options} name='test' label='Email' placeholder='johndoe@gmail.com' inline={boolean('Inline', true)} />
        <Select options={options} name='test' label='Password' type='password' placeholder='*********' inline={boolean('Inline', true)} />
        <Checkbox name='agree' label='I Agree' color='primary' />
        <Button children='SAVE' color='primary' icon='chevron_right' labelPosition='right' />
      </div>
    </Segment>
  ))
  .add('Hide Label', () => <Select options={options} hideLabel label='Label Hidden' placeholder='Label Hidden...' name='test' />)
  .add('Form Message', () => (
    <Select options={options} label='Name' message='You can decorate your form field with a messge.' name='test' />
  ))
  .add('Action w/ Icon', () => (
    <Select options={options} label='SSN' message='Enter your Social Security Number' action={action('Test')} actionIcon='eye' />
  ))
  .add('Action w/ Text', () => (
    <Select
      options={options} 
